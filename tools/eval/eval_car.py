...
def draw_bbox(score_thr, anno_json, pred_json, img_path, result_path):
    score_thr = {0:0.4, 1: 0.5, 2: 0.5, 3: 2, 4:2}
...
if __name__ == '__main__':
    anno_json = '/root/paddlejob/workspace/env_run/lvfeng/output/road_scene_det_eval_0828/test_0829.json'    # 标注框json路径
    pred_json = 'bbox.json'    # 预测框json路径
    roi_path = ''     # 道路roi路径

    # 计算p，r值 ， iou_thr：iou阈值 ， recall_thrs: recall值, cat_ids:要评测的类别
    recall_thrs = np.linspace(1, 0.5, int(np.round((1 - 0.5) / 0.05)) + 1, endpoint=True)  
    compute_pr(iou_thr = 0.1, recall_thrs=recall_thrs, cat_ids = [0,1,2,3,4], anno_json = anno_json, pred_json = pred_json, roi_path=roi_path, is_c_language=False, merge_cate=False)

    # # 预测结果可视化
    # img_path = '/root/paddlejob/workspace/env_run/lvfeng/output/车端抛洒物val20240124/'
