...
def compute_pr(iou_thr, recall_thrs, cat_ids, anno_json, pred_json, roi_path, is_c_language, merge_cate = False, classwise=True):
    '''
    修改：
    1. 标注gt面积小于24*16的忽略
    ...
    if classwise:
        ...
        pre_precisions = eval.eval['precision']
        
        iou_index_cls = np.where(eval.params.iouThrs == 0.5)[0]
        cat_ids = anno.getCatIds()
        # precision: (iou, recall, cls, area range, max dets)
        assert len(cat_ids) == pre_precisions.shape[2]
        results_per_category = []
        for idx, catId in enumerate(cat_ids):
            # area range index 0: all area ranges
            # max dets index -1: typically 100 per image
            nm = anno.loadCats(catId)[0]
            pre_precision = pre_precisions[iou_index_cls, :, idx, 0, -1]
            pre_precision = pre_precision[pre_precision > -1]
            if pre_precision.size:
                ap = np.mean(pre_precision)
            else:
                ap = float('nan')
...
def draw_bbox(score_thr, anno_json, pred_json, img_path, result_path):
    ...
    for cat_name in cat_nms:
        ...
        annId = anno.getAnnIds(imgIds=imgId, catIds=catId, iscrowd=None)                  
        print("{:<15} {:<6d}     {:<10d}".format(cat_name, len(imgId), len(annId)))
    ...
    for img_id in ids:
        ...
        if have_bbox or have_anno:
            save_path = os.path.join(result_path, path)
...
if __name__ == '__main__':
    ...
    recall_thrs = np.linspace(1.0, 0.5, int(np.round((1.0 - 0.5) / 0.05)) + 1, endpoint=True)  
    compute_pr(iou_thr = 0.1, recall_thrs=recall_thrs, cat_ids = [0, 1, 2], anno_json = anno_json, pred_json = pred_json, roi_path=roi_path, is_c_language=False, merge_cate=False, classwise=True)

    # # 预测结果可视化
    # img_path = '/root/paddlejob/workspace/env_run/lvfeng/output/抛洒物危险等级1227val/'
    # result_path = '/root/paddlejob/workspace/env_run/lvfeng/PaddleDetection/'
    # score_thr = {0:0.51, 1: 0.51, 2: 0.36, 3: 2, 4:2}
    # draw_bbox(score_thr, anno_json, pred_json, img_path, result_path)
