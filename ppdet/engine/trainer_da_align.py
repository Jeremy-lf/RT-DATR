...
__all__ = ['Trainer_DA_RTDETR_Align']


class Trainer_DA_RTDETR_Align(Trainer):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id in range(len(self.loader)):
                ...
                with model.no_sync():
                    ...
                    elif self.cfg.da_method == 'o2net_rtdetr_align':
                        criterion = create('DINOLoss')
                        outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_src, \
                                domain_outs_src, domain_labels_src, masks_src, hs_src, src_num_denoising, src_da_loss_cls, src_da_loss_loc = model(data_sup)
                        
                        outputs_bbox_tgt, outputs_logit_tgt, gt_bbox_tgt, gt_class_tgt, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_tgt, \
                                domain_outs_tgt, domain_labels_tgt, masks_tgt, hs_tgt, tgt_num_denoising, tgt_da_loss_cls, tgt_da_loss_loc = model(data_unsup)
                    ...
                    # inconsistency align
                    da_cls_loss = 1.0 * (src_da_loss_cls - tgt_da_loss_cls)
                    da_loc_loss = 1.0 * (src_da_loss_loc - tgt_da_loss_loc)

                    losses_dict['ins_align'] = da_cls_loss + da_loc_loss
