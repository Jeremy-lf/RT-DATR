...
__all__ = ['Trainer_DA_RTDETR_Backbone_Encoder_Instance_DN_CMT']


class Trainer_DA_RTDETR_Backbone_Encoder_Instance_DN_CMT(Trainer):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id in range(len(self.loader)):
                ...
                with model.no_sync():
                    if self.cfg.da_method == 'rtdetr_backbone_encoder_instance_dn_cmt':
                        ...
                        outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src, \
                            dn_out_bboxes_src, dn_out_logits_src, dn_meta_src, gt_score_src, \
                                domain_outs_enc_src, domain_labels_enc_src, masks_src, hs_srcs, \
                                    src_num_denoising, domain_outs_backbone_src, domain_labels_backbone_src, out_instance_query_src = model(data_sup)
                        
                        outputs_bbox_tgt, outputs_logit_tgt, gt_bbox_tgt, gt_class_tgt, \
                            dn_out_bboxes_tgt, dn_out_logits_tgt, dn_meta_tgt, gt_score_tgt, \
                                domain_outs_enc_tgt, domain_labels_enc_tgt, masks_tgt, hs_tgts, \
                                    tgt_num_denoising, domain_outs_backbone_tgt, domain_labels_backbone_tgt, out_instance_query_tgt = model(data_unsup)

                        losses_dict = criterion(outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src,
                            dn_out_bboxes=dn_out_bboxes_src, dn_out_logits=dn_out_logits_src, dn_meta=dn_meta_src, gt_score=gt_score_src)
                    
                    
                    # align target decoder aux layer
                    logit_mean = paddle.mean(outputs_logit_tgt, axis=0).detach()
                    boxes_mean = paddle.mean(outputs_bbox_tgt, axis=0).detach()
                    
                    loss_cmt_cls = 0
                    for i in range(len(outputs_logit_tgt)):
                        aux_logit = outputs_logit_tgt[i] # Bx100xnum_cls
                        prob_mean = 0.5 * (F.softmax(aux_logit, axis=-1) + F.softmax(logit_mean, axis=-1))
                        loss_cmt_cls +=  0.5 * (F.kl_div(F.log_softmax(logit_mean, axis=-1), prob_mean) + \
                            F.kl_div(F.log_softmax(aux_logit, axis=-1), prob_mean))
                    losses_dict['loss_cmt_cls'] = loss_cmt_cls
                    
                    loss_cmt_box = 0
                    for i in range(len(outputs_bbox_tgt)):
                        aux_box = outputs_bbox_tgt[i]
                        loss_cmt_box += F.l1_loss(aux_box, boxes_mean)
                    
                    losses_dict['loss_cmt_box'] = loss_cmt_box
                    
                    # instance query da focal loss
                    src_domain = paddle.zeros_like(out_instance_query_src)
                    tgt_domain = paddle.ones_like(out_instance_query_tgt)

                    # src_space_query_loss = F.binary_cross_entropy_with_logits(out_instance_query_src, \
                    #     src_domain, reduction='mean')
                    # tgt_space_query_loss = F.binary_cross_entropy_with_logits(out_instance_query_tgt, \
                    #     tgt_domain, reduction='mean')

                    # losses_dict['instance_loss'] = (src_space_query_loss + tgt_space_query_loss)
                    src_space_query_loss = F.binary_cross_entropy_with_logits(out_instance_query_src, \
                        src_domain, reduction='none')
                    tgt_space_query_loss = F.binary_cross_entropy_with_logits(out_instance_query_tgt, \
                        tgt_domain, reduction='none')

                    src_prob = F.sigmoid(out_instance_query_src)
                    src_p_t = src_prob * src_domain + (1 - src_prob) * (1 - src_domain)
                    src_loss = src_space_query_loss * ((1 - src_p_t) ** 2)
                    
                    tgt_prob = F.sigmoid(out_instance_query_tgt)
                    ...
                    # encoder global da loss
                    loss_global_encoder = 0
                    for i in range(len(domain_outs_enc_src)):
                        domain_out_dec_feat_src = domain_outs_enc_src[i]
                        domain_dec_label_src = domain_labels_enc_src[i]

                        domain_out_dec_feat_tgt = domain_outs_enc_tgt[i]
                        domain_dec_label_tgt = domain_labels_enc_tgt[i]

                        global_src_loss = F.binary_cross_entropy_with_logits(domain_out_dec_feat_src, domain_dec_label_src, reduction='mean')
                        global_tgt_loss = F.binary_cross_entropy_with_logits(domain_out_dec_feat_tgt, domain_dec_label_tgt, reduction='mean')

                        loss_global_encoder += global_src_loss + global_tgt_loss
                    losses_dict["global_loss"] = loss_global_encoder

                    
                    
                    # backbone local pixel-level adv loss
                    pseudo = None
                    ...
                    loss_da = 0
                    loss_global_da = 0
                    for l in range(len(domain_outs_backbone_src)):
                        domain_out_src = domain_outs_backbone_src[l]   # torch.Size([2, 2, 64, 64])
                        domain_label_src = domain_labels_backbone_src[l]   # torch.Size([1, 64, 64])
                        mask_src = masks_src[l]
                        domain_out_tgt = domain_outs_backbone_tgt[l]  # torch.Size([2, 2, 72, 72])
                        domain_label_tgt = domain_labels_backbone_tgt[l]  # torch.Size([1, 72, 72])
                    ...
                    # DA total loss
                    losses_dict["loss_backbone_da"] = 2.0 * loss_da + loss_global_da  # R34->1.0 R50->1.5
