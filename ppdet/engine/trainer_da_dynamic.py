...
__all__ = ['Trainer_DA_RTDETR_Space']


class Trainer_DA_RTDETR_Space(Trainer):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id in range(len(self.loader)):
                ...
                with model.no_sync():
                    ...
                    elif self.cfg.da_method == 'o2net_rtdetr_space':
                        criterion = create('DINOLoss')
                        outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_src, \
                                domain_outs_src, domain_labels_src, masks_src, hs_src, src_num_denoising, outputs_space_query_src = model(data_sup)
                        
                        outputs_bbox_tgt, outputs_logit_tgt, gt_bbox_tgt, gt_class_tgt, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_tgt, \
                                domain_outs_tgt, domain_labels_tgt, masks_tgt, hs_tgt, tgt_num_denoising, outputs_space_query_tgt = model(data_unsup)

                        losses_dict = criterion(outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src,
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_src)
                    
                    # decoder da loss
                    src_domain = paddle.zeros_like(outputs_space_query_src)
                    tgt_domain = paddle.ones_like(outputs_space_query_tgt)

                    src_space_query_loss = F.binary_cross_entropy_with_logits(outputs_space_query_src, \
                        src_domain, reduction='none')
                    tgt_space_query_loss = F.binary_cross_entropy_with_logits(outputs_space_query_tgt, \
                        tgt_domain, reduction='none')

                    src_prob = F.sigmoid(outputs_space_query_src)
                    src_p_t = src_prob * src_domain + (1 - src_prob) * (1 - src_domain)
                    src_loss = src_space_query_loss * ((1 - src_p_t) ** 2)
                    
                    tgt_prob = F.sigmoid(outputs_space_query_tgt)
                    ...
                    losses_dict['space_loss'] = space_loss * 0.5
