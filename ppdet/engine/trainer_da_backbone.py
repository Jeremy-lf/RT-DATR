...
__all__ = ['Trainer_DA_RTDETR_Backbone']


class Trainer_DA_RTDETR_Backbone(Trainer):
                ...
                with model.no_sync():
                    if self.cfg.da_method == 'o2net_detr':
                        ...
                        outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src, \
                            domain_outs_enc_src, domain_labels_enc_src, masks_src, hs_src = model(data_sup)
                        outputs_bbox_tgt, outputs_logit_tgt, gt_bbox_tgt, gt_class_tgt, \
                            domain_outs_enc_tgt, domain_labels_enc_tgt, masks_tgt, hs_tgt = model(data_unsup)
                        losses_dict = criterion(outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src)
                    
                    elif self.cfg.da_method == 'o2net_rtdetr_backbone':
                        criterion = create('DINOLoss')
                        outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_src, \
                                domain_outs_dec_src, domain_labels_dec_src, masks_src, hs_src, \
                                    src_num_denoising, domain_outs_enc_src, domain_labels_enc_src = model(data_sup)
                        
                        outputs_bbox_tgt, outputs_logit_tgt, gt_bbox_tgt, gt_class_tgt, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_tgt, \
                                domain_outs_dec_tgt, domain_labels_dec_tgt, masks_tgt, hs_tgt, \
                                    tgt_num_denoising, domain_outs_enc_tgt, domain_labels_enc_tgt = model(data_unsup)
                    ...
                    loss_da = 0
                    loss_global_da = 0
                    for l in range(len(domain_outs_enc_src)):
                        domain_out_src = domain_outs_enc_src[l]   # torch.Size([2, 2, 64, 64])
                        domain_label_src = domain_labels_enc_src[l]   # torch.Size([1, 64, 64])
                        mask_src = masks_src[l]
                        domain_out_tgt = domain_outs_enc_tgt[l]  # torch.Size([2, 2, 72, 72])
                        domain_label_tgt = domain_labels_enc_tgt[l]  # torch.Size([1, 72, 72])
