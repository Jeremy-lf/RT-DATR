...
__all__ = ['Trainer_DA_RTDETR_Weak']


class Trainer_DA_RTDETR_Weak(Trainer):
    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id in range(len(self.loader)):
                ...
                print(data_sup['image'].shape)
                print(data_unsup['image'].shape)
                data_sup['domain'] = 0
                data_unsup['domain'] = 1
                ...
                with model.no_sync():
                    ...
                    elif self.cfg.da_method == 'o2net_rtdetr_weak':
                        criterion = create('DINOLoss')
                        outputs_bbox_src, outputs_logit_src, gt_bbox_src, gt_class_src, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_src, \
                                domain_outs_src, domain_labels_src, masks_src, hs_src, src_num_denoising = model(data_sup)
                        
                        outputs_bbox_tgt, outputs_logit_tgt, gt_bbox_tgt, gt_class_tgt, \
                            dn_out_bboxes, dn_out_logits, dn_meta, gt_score_tgt, \
                                domain_outs_tgt, domain_labels_tgt, masks_tgt, hs_tgt, tgt_num_denoising = model(data_unsup)
                    ...
                    # if paddle.sum(scores_indices).numpy().item() < 1:
                    #     gamma = 1.0
                    # else:
                    #     avg_scores = paddle.sum(scores[scores_indices]).numpy() / paddle.sum(scores_indices).numpy()
                    #     gamma = 0.5 + (1.0 - avg_scores[0])
                    ...
                    # global alignment
                    global_out_src = domain_outs_src[-1]
                    global_label_src = domain_labels_src[-1]

                    global_out_tgt = domain_outs_tgt[-1]
                    global_label_tgt = domain_labels_tgt[-1]

                    global_src_loss = F.binary_cross_entropy_with_logits(global_out_src, global_label_src, reduction='mean')
                    global_tgt_loss = F.binary_cross_entropy_with_logits(global_out_tgt, global_label_tgt, reduction='mean')

                    total_loss = global_src_loss + global_tgt_loss
                    losses_dict["global_loss"] = total_loss

                    # Focal loss
                    # src_prob = F.sigmoid(global_out_src)
                    # tgt_prob = F.sigmoid(global_out_tgt)
                    
                    # src_p_t = src_prob * global_label_src + (1 - src_prob) * (1 - global_label_src)
                    # src_loss = global_src_loss * ((1 - src_p_t) ** 2)

                    # tgt_p_t = tgt_prob * global_label_tgt + (1 - tgt_prob) * (1 - global_label_tgt)
                    # tgt_loss = global_tgt_loss * ((1 - tgt_p_t) ** 2)

                    # global_loss = src_loss.mean() + tgt_loss.mean()

                    # losses_dict["global_loss"] = global_loss
