...
@register
class DA_DINOHead(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss', eval_idx=-1):
        super(DA_DINOHead, self).__init__()
        self.loss = loss
        self.eval_idx = eval_idx

    def forward(self, out_transformer, body_feats, inputs=None):
        (dec_out_bboxes, dec_out_logits, enc_topk_bboxes, enc_topk_logits,
         dn_meta, domain_outs, domain_labels, masks, outputs, num_denoising) = out_transformer
        ...
        if self.training:
            ...
            if 'gt_bbox' in inputs:
                ...
                return out_bboxes, out_logits, None, None, \
                    dn_out_bboxes, dn_out_logits, dn_meta, None, \
                        domain_outs, domain_labels, masks, outputs, None
        else:
