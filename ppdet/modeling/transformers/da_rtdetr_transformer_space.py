...
from cv2 import ellipse, fastNlMeansDenoisingMulti
...
__all__ = ['DA_RTDETRTransformer_Space']
...
class TransformerDecoderLayer(nn.Layer):
    ...
    def forward(self,
        ...
        if self.training and self.space_align:
            space_query = self.space_attn(space_query, tgt, query_pos_embed, attn_mask)
        ...
        return tgt, space_query


class TransformerDecoder(nn.Layer):
    def __init__(self, hidden_dim, decoder_layer, num_layers, eval_idx=-1, space_align=True):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output, space_query = layer(output, ref_points_input, memory,
                           memory_spatial_shapes, memory_level_start_index,
                           attn_mask, memory_mask, query_pos_embed, space_query)
        ...
        if self.training and self.space_align:
            out_space_querys = paddle.concat(space_querys, axis=1)
        else:
            out_space_querys = None

        return paddle.stack(dec_out_bboxes), paddle.stack(dec_out_logits), paddle.stack(outputs), out_space_querys


@register
class DA_RTDETRTransformer_Space(nn.Layer):
    ...
    def __init__(self,
        ...
        super(DA_RTDETRTransformer_Space, self).__init__()
        ...
        self.decoder = TransformerDecoder(hidden_dim, decoder_layer,
                                          num_decoder_layers, eval_idx, space_align)
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        ...
        if self.training and self.space_align:
            space_query = paddle.expand(self.space_query, [target.shape[0], -1, -1])
        # decoder
        out_bboxes, out_logits, outputs, output_space_queyrs = self.decoder(
            target,
        ...
        # DA
        if self.training and self.space_align:
            output_space_queyrs = self.space_D(output_space_queyrs)

        return (out_bboxes, out_logits, enc_topk_bboxes, enc_topk_logits,
                dn_meta, domain_outs, domain_labels, masks, outputs, num_denoising, output_space_queyrs)
