...
__all__ = ['DA_RTDETRTransformer_Align']
...
class TransformerDecoder(nn.Layer):
    def __init__(self, hidden_dim, decoder_layer, num_layers, num_classes, num_aux1, num_aux2, eval_idx=-1):
        super(TransformerDecoder, self).__init__()
        self.layers = _get_clones(decoder_layer, num_layers)
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        self.eval_idx = eval_idx if eval_idx >= 0 else num_layers + eval_idx
        self.cls_aux = MLP(hidden_dim, hidden_dim, num_classes * num_aux1, num_layers=2)
        self.bbox_aux = MLP(hidden_dim, hidden_dim, 4 * num_aux1, num_layers=2)
    ...
    def forward(self,
        ...
        if self.training:
            last_out = outputs[-1]
            cls_score_aux = self.cls_aux(self.grl(last_out))
            cls_score_aux = paddle.reshape(cls_score_aux, [last_out.shape[0], -1, self.num_aux1])
            cls_prob_aux = F.softmax(cls_score_aux, axis=1)
            weight = cls_prob_aux.mean(axis=2)
            cls_prob_aux = F.softmax(cls_prob_aux, axis=2)
            ent = -1.0 * cls_prob_aux * paddle.log(cls_prob_aux + 1e-5)
            ent = ent.sum(axis=2)
            ent *= weight
            da_loss_cls = -1.0 * ent.sum(axis=1).mean()


            bbox_pred_aux = self.bbox_aux(self.grl(last_out))
            bbox_pred_aux = paddle.reshape(bbox_pred_aux, [last_out.shape[0], -1, self.num_aux2])
            da_loss_loc = bbox_pred_aux.std(axis=2).mean(axis=1).mean()


        return paddle.stack(dec_out_bboxes), paddle.stack(dec_out_logits), paddle.stack(outputs), da_loss_cls, da_loss_loc


@register
class DA_RTDETRTransformer_Align(nn.Layer):
    ...
    def __init__(self,
                 ...
                 num_aux1=2,
                 num_aux2=2):
        super(DA_RTDETRTransformer_Align, self).__init__()
        ...
        self.decoder = TransformerDecoder(hidden_dim, decoder_layer,
                                          num_decoder_layers, num_classes, num_aux1, num_aux2, eval_idx)
        ...
        # if space_align:
        #     self.space_query = self.create_parameter(shape=[1, 1, hidden_dim], default_initializer=nn.initializer.Constant(0.0))
        # if channel_align:
        #     # self.channel_query is actually an embedding layer for channel query
        #     # We keep the name for consistency
        #     self.channel_query = nn.Linear(hidden_dim, 1)
        #     self.grl = GradientReversalLayer()
        # if instance_align:
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        ...
        # decoder
        out_bboxes, out_logits, outputs, da_loss_cls, da_loss_loc = self.decoder(
            target,
        ...
        return (out_bboxes, out_logits, enc_topk_bboxes, enc_topk_logits,
                dn_meta, domain_outs, domain_labels, masks, outputs, num_denoising, da_loss_cls, da_loss_loc)
