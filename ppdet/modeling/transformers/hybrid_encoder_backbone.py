...
__all__ = ['HybridEncoder_Backbone']
...
@register
@serializable
class HybridEncoder_Backbone(nn.Layer):
    ...
    def __init__(self,
                 ...
                 trt=False,
                 eval_size=None):
        super(HybridEncoder_Backbone, self).__init__()
        ...
        self.backbone_da = _ImageDA(hidden_dim)
        self._reset_parameters()
    ...
    def forward(self, feats, gt_meta=None, for_mot=False, is_teacher=False):
        assert len(feats) == len(self.in_channels)
        # get projection features
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]

        # backbone feat alignment
        if 'domain' in gt_meta:
            ...
            for i in range(len(feats)):
                ...
                domain_out, domain_label = self.backbone_da(src_domian, domain)
        ...
        return outs, domain_outs, domain_labels
