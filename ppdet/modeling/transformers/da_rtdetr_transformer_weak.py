...
__all__ = ['DA_RTDETRTransformer_Weak']
...
class TransformerDecoder(nn.Layer):
    ...
    def forward(self,
        ...
        return paddle.stack(dec_out_bboxes), paddle.stack(dec_out_logits), paddle.stack(outputs)


@register
class DA_RTDETRTransformer_Weak(nn.Layer):
    ...
    def __init__(self,
        ...
        super(DA_RTDETRTransformer_Weak, self).__init__()
        ...
        self.feat_grl = GradientReversalLayer()
        self.feat_discriminator = MLP(hidden_dim, hidden_dim, 2, 3)
        self._reset_parameters()
    ...
    def _get_encoder_input(self, feats, pad_mask, gt_meta):
        ...
        if 'domain' in gt_meta:
            ...
            # global aignment
            global_domian = self.feat_grl(proj_feats[-1].flatten(2).transpose([0,2,1]))
            global_domian = self.feat_discriminator(global_domian)
            
            if gt_meta['domain'] == 0:
                global_labels = paddle.ones_like(global_domian)
            else:
                global_labels = paddle.zeros_like(global_domian)
            domain_outs.append(global_domian)
            domain_labels.append(global_labels)
        else:
            domain_outs = None
            domain_labels = None
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        ...
        # decoder
        out_bboxes, out_logits, outputs = self.decoder(
            target,
        ...
        return (out_bboxes, out_logits, enc_topk_bboxes, enc_topk_logits,
                dn_meta, domain_outs, domain_labels, masks, outputs, num_denoising)
