...
def get_contrastive_denoising_training_group(targets,
    ...
    max_gt_num = max(num_gts)
    if max_gt_num == 0:
        print("No ground truth in this batch, skip!")
        return None, None, None, None
    ...
    positive_gt_mask = 1 - negative_gt_mask  # bsx(2*num_group*max_gt_num,1)
...
def get_contrastive_denoising_training_group_da(targets,
                                             num_classes,
                                             num_queries,
                                             class_embed,
                                             num_denoising=100,
    ...
    return input_query_class, num_denoising, input_query_bbox, attn_mask, dn_meta
...
class DomainAttention(nn.Layer):
    ...
    def forward(self, query, src, pos=None, attn_mask=None):
        """ Args:
        ...
        """
        r_query = self.cross_attn(
            query=paddle.transpose(query, perm=[1, 0, 2]),
            key=paddle.transpose(self.grl(self.with_pos_embed(src, pos)), perm=[1, 0, 2]),
            value=paddle.transpose(self.grl(src), perm=[1, 0, 2]),
            attn_mask=None,
        )
        # print(r_query.shape)
        query = query + self.dropout1(paddle.transpose(r_query, perm=[1, 0, 2]))
