...
from .utils import (_get_clones, get_sine_pos_embed,
                    get_contrastive_denoising_training_group_da, inverse_sigmoid)

from .utils import DomainAttention
from .DA import GradientReversalLayer
from .DA import _ImageDA

__all__ = ['DA_RTDETRTransformer_Global_Instance']
...
class TransformerDecoderLayer(nn.Layer):
    def __init__(self,
        ...
        self.space_attn = DomainAttention(d_model, n_head, dropout)
    ...
    def forward(self,
        ...
        if self.training and self.instance_align:
            instance_query = self.space_attn(instance_query, tgt, query_pos_embed)
...
class TransformerDecoder(nn.Layer):
    def __init__(self, hidden_dim, decoder_layer, num_layers, eval_idx=-1, instance_align=True):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output, instance_query = layer(output, ref_points_input, memory,
                           memory_spatial_shapes, memory_level_start_index,
                           attn_mask, memory_mask, query_pos_embed, instance_query)
            outputs.append(output)
            # instance_querys.append(instance_query)
        ...
        # if self.training and self.instance_align:
        #     out_instance_querys = paddle.concat(instance_querys, axis=1)
        # else:
        #     out_instance_querys = None
        return paddle.stack(dec_out_bboxes), paddle.stack(dec_out_logits), paddle.stack(outputs), instance_query


@register
class DA_RTDETRTransformer_Global_Instance(nn.Layer):
    ...
    def __init__(self,
                 ...
                 eps=1e-2,
                 instance_align=True):
        super(DA_RTDETRTransformer_Global_Instance, self).__init__()
        ...
        # encoder adversial alignment
        self.grl = GradientReversalLayer()
        self.feat_discriminator = MLP(hidden_dim, hidden_dim, 2, num_layers=3)

        # instance da method
        self.discriminator = _ImageDA(dim=hidden_dim)
        if self.instance_align:
            self.space_D = MLP(hidden_dim, hidden_dim, 2, num_layers=3)
            self.instance_query = self.create_parameter(shape=[1, 1, hidden_dim], default_initializer=nn.initializer.Constant(0.0))
    ...
    def _get_encoder_input(self, feats, pad_mask, gt_meta):
        ...
        # encoder global da
        if 'domain' in gt_meta:
            domain_outs = []
            domain_labels = []
            for i in range(self.num_levels):
                global_domian = self.grl(proj_feats[i].flatten(2).transpose([0,2,1]))
                global_domian = self.feat_discriminator(global_domian)
                if gt_meta['domain'] == 0:
                    domain_label = paddle.zeros_like(global_domian)
                else:
                    domain_label = paddle.ones_like(global_domian)
                domain_outs.append(global_domian)
        ...
        return (feat_flatten, spatial_shapes, level_start_index, domain_outs, domain_labels, masks)

    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        # input projection and embedding
        (memory, spatial_shapes, level_start_index,
            domain_outs, domain_labels, masks) = self._get_encoder_input(feats, pad_mask, gt_meta)

        # t-sne for encoder
        # import time
        # import numpy as np
        # t_begin = np.round(time.time(), 4)
        # np.save('da_tgt_encoder_tsne/' + str(t_begin) + '.npy', memory.numpy())

        # prepare denoising training
        if self.training:
            # get denoising training group for supervised learning
            if 'gt_bbox' in gt_meta:
                denoising_class, num_denoising, denoising_bbox_unact, attn_mask, dn_meta = \
                    get_contrastive_denoising_training_group_da(gt_meta,
                                                self.num_classes,
                                                self.num_queries,
                ...
                denoising_class, num_denoising, denoising_bbox_unact, attn_mask, dn_meta = None, 0, None, None, None
        else:
            denoising_class, num_denoising, denoising_bbox_unact, attn_mask, dn_meta = None, 0, None, None, None
        ...
        instance_query = None
        if self.training and self.instance_align:
            instance_query = paddle.expand(self.instance_query, [target.shape[0], -1, -1])

        # decoder
        out_bboxes, out_logits, outputs, out_instance_query = self.decoder(
            target,
        ...
        return (out_bboxes, out_logits, enc_topk_bboxes, enc_topk_logits,
                dn_meta, domain_outs, domain_labels, masks, outputs, num_denoising, out_instance_query)
