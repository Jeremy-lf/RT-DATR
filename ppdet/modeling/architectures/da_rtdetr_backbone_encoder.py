...
__all__ = ['DA_RTDETR_Backbone_Encoder']
# Deformable DETR, DINO use the same architecture as DETR


@register
class DA_RTDETR_Backbone_Encoder(BaseArch):
    ...
    def __init__(self,
        ...
        super(DA_RTDETR_Backbone_Encoder, self).__init__()
    ...
    def _forward(self):
        ...
        out_transformer = \
        (out_bboxes, out_logits, enc_topk_bboxes, enc_topk_logits, dn_meta, domain_outs_dec, domain_labels_dec, masks, outputs, num_denoising) = \
        ...
        if self.training:
            out_bboxes, out_logits, gt_bbox, gt_class, \
                dn_out_bboxes, dn_out_logits, dn_meta, _, \
                    domain_outs_dec, domain_labels_dec, masks, outputs, num_denoising  = self.detr_head(
            ...
            return out_bboxes, out_logits, gt_bbox, gt_class, \
                   dn_out_bboxes, dn_out_logits, dn_meta, None, \
                       domain_outs_dec, domain_labels_dec, masks, outputs, num_denoising, domain_outs_enc, domain_labels_enc
        else:
